...
function recordsForFilter(records){ return state.confidence==="all"?records:records.filter((r)=>normalize(r.Rel)===state.confidence); }
...
function buildConfidenceOptions(){
  ...
  elements.confidenceOptions.replaceChildren(); ["all",...sorted].forEach((value,index)=>{ const label=document.createElement("label");label.className="radio-row"; const input=document.createElement("input");input.type="radio";input.name="confidence";input.value=value;input.checked=index===0; const span=document.createElement("span");span.textContent=value==="all"?"All":value; label.append(input,span);elements.confidenceOptions.append(label); });
  elements.confidenceOptions.addEventListener("change",(event)=>{if(event.target.name!=="confidence")return;state.confidence=event.target.value;rebuildAllLayers();showStatus(`Confidence filter: ${event.target.closest("label").innerText.trim()}.`);});
}
